...
- Починен приём заявок: с момента запуска форма теряла все обращения. `BITRIX24_WEBHOOK_URL` не был прописан на хостинге и вдобавок не прокидывался Dockerfile'ом, а проект Supabase перестал существовать (домен отдаёт NXDOMAIN). В CRM не было ни одного лида с сайта.
- Вебхук передаётся контейнеру рантайм-переменной, а не build-аргументом: через ARG секрет оседал в слоях образа, хотя даёт полный доступ к CRM.
- Мёртвый Supabase убран из кода, переменных и Dockerfile.
- Добавлен резервный канал: если вебхук не сработал, заявка уходит письмом на ящик, подключённый к CRM (`site/src/app/api/lead/mail.ts`, SMTP Яндекса). Письмо отправляется только при отказе вебхука — оба канала ведут в одну CRM и параллельная отправка давала бы по два лида.
- `GET /api/lead` показывает состояние приёмников без секретов; отказ каждого приёмника и потерянная заявка целиком пишутся в stderr — раньше поломка была не видна вообще.
- Проверено на проде сквозным путём: заявка создаёт лид с разложенными полями, дубля из письма не возникает, тестовые лиды удалены.
...
- Фото в секции `Производство` заменено на студийный кадр линии (1448×1086) и переведено на пайплайн: было 342×229 JPEG. Пропорция блока взята от кадра (4:3) вместо зашитой `342/229` — размера старого файла.
